...
class MDCountIndicator extends Command
{
    ...
    public function handle(IndicatorMongoService $mongo)
    {
      ...
      $pipelineIndustries = array( // base array
        array(
            '$group' => array(
                "_id" => '$industries',
                "total"   => array('$sum'=>1),
           ...
           $pipelineGroup = array( // base array
            array(
                '$group' => array(
                    "_id" => '$groups',
                    "total"   => array('$sum'=>1),
        ...
        foreach ( $countIndustries as $value) {
            ...
            foreach ($pieces as $industrieskey => $industriesvalue) {
                $Indicatorindustries = IndicatorSummaryYear::where('type', 'industries')->where('industries_name', $industriesvalue)->first();
                if(!$Indicatorindustries){
                    $IndicatorSummaryYear = new IndicatorSummaryYear;
                    $IndicatorSummaryYear->type =  'industries';
                    $IndicatorSummaryYear->status = 0;
                    $IndicatorSummaryYear->industries_name = $industriesvalue;
                    $IndicatorSummaryYear->industries_count = $value["total"];
                    $IndicatorSummaryYear->save();

                }else{
                    $Indicatorindustries->industries_count =  $value["total"];
                    $Indicatorindustries->save();

                }
            }
        }
        foreach ( $countpipelineGroup as $value) {
            $pizza  = $value["_id"];
            $pieces = explode(",", $pizza);
            foreach ($pieces as $industrieskey => $industriesvalue) {
