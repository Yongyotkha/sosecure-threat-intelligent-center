...
use App\Helpers\ImageCompareHelper;

class WebDefacementProccess_dev extends Command
{
    /**
    ...
    protected $signature = 'app:WebDefacementProccess_dev';
     ...
     * @var string
     */
    protected $description = 'WebDefacementProccess_dev';
    ...
    {
      Log::info('[WebDefacementProccess] Starting at ' . now());
        ...
        if ($WebdefacmentSetting_data) {
          ...
          if ($WebdefacmentDataOriginal_data) {
            ...
            if (count($WebdefacmentImageMark_check) > 0) {
              ...
              $comparePath = base_path()."/public/images/webdefacment_mages/".$site_id."/".$url_id."/";
              $imageOriginal = $comparePath . 'image_original.png';
              $imageCompare = $comparePath . $image_name . "_Defacement_Now.png";

              $retryCount = 0;
              while ((!file_exists($imageOriginal) || !file_exists($imageCompare)) && $retryCount < 25) {
                  usleep(200000); // รอ 0.2 วินาที
                  $retryCount++;
              }
      ...
      //  $orignal_ = '';
      // $orignal_ = base_path() . "/public/images/webdefacment_mages/".$site_id."/".$url_id."/image_original.png";
...
private function compareImage2($dirPath, $imgSourceFilename, $imgCompareFilename)
{
    $imgSourcePath = $dirPath . $imgSourceFilename;
    $imgComparePath = $dirPath . $imgCompareFilename;

    Log::info("[compareImage2] เปรียบเทียบภาพ:");
    Log::info(" - Source: $imgSourcePath");
    Log::info(" - Compare: $imgComparePath");

    // ตรวจสอบขนาดภาพก่อน
    if (!file_exists($imgSourcePath) || !file_exists($imgComparePath)) {
        Log::error("[compareImage2] ไม่พบไฟล์ภาพ");
        return [
            "image1Hash" => null,
            "image2Hash" => null,
            "diff" => 100
    ...
    $maxSafeHeight = 10000; // กำหนดเพดานความสูงที่ระบบเดิมรับได้

      if ($imgHeight > $maxSafeHeight) {
      Log::warning("[compareImage2] ภาพสูงเกิน $maxSafeHeight px → ใช้ slice เปรียบเทียบ");

      $sliceResult = ImageCompareHelper::compareLargeImagesBySlice($imgSourcePath, $imgComparePath, 2000);

        return [
            "image1Hash" => $sliceResult['image1Hash'],
            "image2Hash" => $sliceResult['image2Hash'],
            "diff" => $sliceResult['percentDiff']
        ];
    }


    // ✅ ภาพไม่ใหญ่ → ใช้ระบบเดิม
    try {
        $compareMachine = new compareImages($imgSourcePath);

        $image1 = $compareMachine->createImage($imgSourcePath);
        $image2 = $compareMachine->createImage($imgComparePath);

        if (!$image1 || !$image2) {
            Log::error("[compareImage2] โหลดภาพล้มเหลว");
            return [
                "image1Hash" => null,
                "image2Hash" => null,
                "diff" => 100
        ...
        $image1Hash = $compareMachine->getHasString();
        $image2Hash = $compareMachine->hasStringImage($imgComparePath);
        $diff = $compareMachine->compareHash($image2Hash);

        Log::info("[compareImage2] diff: $diff");
        ...
        Log::error("[compareImage2] Exception: " . $e->getMessage());
        return [
            "image1Hash" => null,
            "image2Hash" => null,
            "diff" => 100
