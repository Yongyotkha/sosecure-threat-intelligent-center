...
//echo json_encode($strJsonFileContents);
return 0;
echo "OUT";
        ...
        foreach ($json_data['cve']['affects']['vendor']['vendor_data'] as $vendorkey => $vendor) {
            ...
            foreach ($vendor['product']['product_data'] as $productkey => $productvalue) {
                ...
                foreach ($array_version as $versionkey => $versionvalue) {
                    echo PHP_EOL . 'check...';
                    $sql_samename = "SELECT namecve FROM data_cveven WHERE namecve = '" . $CVE_Code . "' and title='" . $vendor_name . "' and vendor='" . $product_name . "' and version='" . $versionvalue . "'";
...
//=================================
        $sql_samename = "SELECT namecve FROM data_datacve WHERE namecve = '" . $CVE_Code . "'";
        $result1 = mysqli_query($conn, $sql_samename) or die(mysqli_error());
        $num = mysqli_num_rows($result1);
//$num = 0;
        if ($num > 0) {
...
function insert_nvd($conn, $add_name, $add_published, $add_modified, $add_descript, $add_cvsssore, $add_severity, $add_pub_date, $created_at)
{
    ...
    $insertdata = "INSERT INTO data_datacve (namecve, published, modified, description, cvss_score, severity, updated_at, created_at)
  VALUES  ('" . $add_name . "',
  '" . $add_published . "',
  '" . $add_modified . "',
  '" . $add_descript . "',
...
{
    $updatedata = "UPDATE data_datacve SET
