...
class compareImagesDev
{
    ...
    {
        // กรณี: ถ้าเป็น path จริง
        if (is_string($i) && file_exists($i)) {
            $imageSize = getimagesize($i);
            if (!$imageSize) {
                \Log::error("[createImage] getimagesize ล้มเหลว: $i");
                return false;
            }

            
            // ✅ ให้แจ้งเตือน แต่ไม่บล็อก
            if ($imageSize[0] > 4000 || $imageSize[1] > 4000) {
                \Log::warning("[createImage] ภาพใหญ่ (แต่อนุญาต): {$imageSize[0]}x{$imageSize[1]} | $i");
                // ยังโหลดภาพต่อ
            }


            $mime = $this->mimeType($i);
            \Log::info("[createImage] โหลดภาพจาก path | type={$mime[2]} | $i");
            ...
            switch ($mime[2]) {
                ...
                case 'png':
                    ...
                    break;
                default:
                    \Log::warning("[createImage] ไม่รองรับไฟล์ประเภท: {$mime[2]}");
                    return false;
            }

            if (!$img) {
                \Log::error("[createImage] โหลดภาพจาก path ล้มเหลว: $i");
                return false;
            }
        ...
        // กรณี: เป็น binary string (base64 หรือ image buffer)
        if (is_string($i)) {
            \Log::info("[createImage] โหลดภาพจาก binary string");
            $img = @imagecreatefromstring($i);

            if (!$img) {
                \Log::error("[createImage] imagecreatefromstring ล้มเหลว");
                return false;
            }
        ...
        \Log::error("[createImage] รูปแบบ input ไม่รองรับ: " . gettype($i));
        return false;
    }
    ...
    {
        if (!$image) {
            Log::error("[hasStringImage] สร้างภาพไม่สำเร็จจาก input");
            return null;
        }
