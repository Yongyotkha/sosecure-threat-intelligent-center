...
class WebDefacementProccess_linux extends Command
{
  /**
  ...
  protected $signature = 'app:WebDefacementProccess_linux';
      ...
      if ($WebdefacmentSetting_data) {
        ...
        if ($WebdefacmentDataOriginal_data) {
          ...
          if ($this->is_url($url)) {
            ...
            if ($webdefacment_id == 173 || $webdefacment_id == 174) {
  ...
  private function getHtml3($url)
  {
    ...
    try {
      ...
      // ✅ ปิด Chrome ที่ค้างไว้ก่อน
      try {
        exec("pkill -f 'chrome --headless' > /dev/null 2>&1 &");
      } catch (\Throwable $ex) {
        \Log::warning("Chrome cleanup failed: " . $ex->getMessage());
      }
      ...
      \Log::error("Puphpeteer Error: " . $e->getMessage());
