            ...
            if ($data === false) {
                ...
                if ($data['data']['menu'] !== 'vulnerabilities') {
                    ...
                    if ($search == 1) {
                        ...
                        return response()->json(['error' => "Ping", 'status_code' => '200', 'data' => 1]);
                    ...
                    if ($group == 2) {
                        ...
                        $CVEMappingAssets_data = CVEMappingAssets::query()
                            ...
                            ->when(!empty($request->site), fn($q) => $q->where('data_datacve_mapping_assets.site_id', $request->site))
                            ->when(!empty($request->vendor), fn($q) => $q->where('cve_assets.vendor', $request->vendor))
                            ->when(!empty($request->title), fn($q) => $q->where('cve_assets.title', $request->title))
                            ->when(!empty($request->assets), fn($q) => $q->where('cve_assets.title', $request->assets))
                            ->when(!empty($request->version) && $request->version !== '*', fn($q) => $q->where('cve_assets.version', $request->version))
                            ->when(!empty($request->edition) && $request->edition !== '*', fn($q) => $q->where('cve_assets.edition', $request->edition))
                            ->when(
        ...
        try {
            ...
            if ($data === false) {
                ...
                if ($data['data']['menu'] !== 'vulnerabilities') {
                    ...
                    if (!empty($isDatesSearch) && $startDate && $endDate) {
