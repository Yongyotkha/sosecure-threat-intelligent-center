                    ...
                    if (isset($data['fx_invoices'])) {
                        foreach ($data['fx_invoices']->where('client', $client['co_id']) as $invoice) {
                            $inv = Invoice::create([
                                ...
                                'due_date'      => validateDate($invoice['due_date']) ? dateParser($invoice['due_date'])->toDateTimeString() : now(),
                                'notes'         => $invoice['notes'],
                                'tax'           => $invoice['tax'],
                                'tax2'          => $invoice['tax2'],
                            ...
                            if (isset($data['fx_payments'])) {
                                foreach ($data['fx_payments']->where('invoice', $invoice['inv_id']) as $payment) {
                                    $inv->payments()->create([
                                        ...
                                        'notes'            => $payment['notes'],
                                        'payment_date'     => validateDate($payment['payment_date']) ? dateParser($payment['payment_date'])->toDateTimeString() : now(),
                    ...
                    if (isset($data['fx_estimates'])) {
                        foreach ($data['fx_estimates']->where('client', $client['co_id']) as $estimate) {
                            $est = Estimate::create([
                                ...
                                'client_id'    => $company->id,
                                'due_date'     => validateDate($estimate['due_date']) ? dateParser($estimate['due_date'])->toDateTimeString() : now(),
                    ...
                    if (isset($data['fx_expenses'])) {
                        foreach ($data['fx_expenses']->where('project', 0)->where('client', $client['co_id']) as $exp) {
                            Expense::create([
                                ...
                                'client_id'    => $company->id,
                                'amount'       => $exp['amount'],
                                'expense_date' => validateDate($exp['expense_date']) ? dateParser($exp['expense_date']) : now(),
                    ...
                    if (isset($data['fx_projects'])) {
                        foreach ($data['fx_projects']->where('client', $client['co_id']) as $project) {
                            $pr = Project::create([
                                ...
                                'start_date'        => validateDate($project['start_date']) ? dateParser($project['start_date'])->toDateTimeString() : now(),
                                'due_date'          => validateDate($project['due_date']) ? dateParser($project['due_date'])->toDateTimeString() : now(),
                            ...
                            if (isset($data['fx_expenses'])) {
                                foreach ($data['fx_expenses']->where('project', $project['project_id']) as $ex) {
                                    $pr->expenses()->create([
                                        ...
                                        'amount'       => $ex['amount'],
                                        'expense_date' => validateDate($ex['expense_date']) ? dateParser($ex['expense_date'])->toDateTimeString() : now(),
                                        'currency'     => $company->currency,
                                        'notes'        => $ex['notes'],
                            ...
                            if (isset($data['fx_milestones'])) {
                                foreach ($data['fx_milestones']->where('project', $project['project_id']) as $milestone) {
                                    $pr->milestones()->create([
                                        ...
                                        'start_date'     => validateDate($milestone['start_date']) ? dateParser($milestone['start_date'])->toDateTimeString() : now(),
                                        'due_date'       => validateDate($milestone['due_date']) ? dateParser($milestone['due_date'])->toDateTimeString() : now(),
                                    ]);
                                }
                            }
                            ...
                            if (isset($data['fx_tasks'])) {
                                foreach ($data['fx_tasks']->where('project', $project['project_id']) as $task) {
                                    $t = $pr->tasks()->create([
                                        ...
                                        'start_date'      => validateDate($task['start_date']) ? dateParser($task['start_date'])->toDateTimeString() : now(),
                                        'due_date'        => validateDate($task['due_date']) ? dateParser($task['due_date'])->toDateTimeString() : now(),
                                        'user_id'         => User::role('admin')->first()->id,
                                        'reminded_at'     => now()->toDateTimeString(),
                                    ]);
                                    // Create team
