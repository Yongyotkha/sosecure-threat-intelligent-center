...
class NewsController extends Controller
{
    ...
    public function news_detail_code($code)
    {
        ...
        if($cate_id_all) {
            ...
            if($rss_news_id_array) {
                $RSSNews_last10 = RSSNews::whereIn('id', $rss_news_id_array)->where('status',1)->where('public_date', '<=', Carbon::now())->orderBy('public_date','DESC')->limit(10)->get();
                // dd($RSSNews_last10);
            }
    ...
    public function jqueryLoadMoreNews(Request $request){
        ...
        $news = RSSNews::where('save_draft', 0)->where('status', 1)->where('public_date', '<=', Carbon::now())->orderBy('public_date','desc')->paginate(5);//->get()
