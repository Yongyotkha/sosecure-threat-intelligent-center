...
abstract class ProjectsController extends Controller
{
    ...
    public function tableData()
    {
        ...
        return DataTables::eloquent($model)
            ...
            ->editColumn('client_id', function ($project) {
                return $project->client_id ? '<a href="' . route('clients.view', $project->client_id) . '">' . str_limit($project->company->name, 20) . '</a>' : 'N/A';
            })
            ...
            ->editColumn('used_budget', function ($project) {
                $color = $project->used_budget > 100 ? 'danger' : 'success';
    ...
    public function applyFilter()
    {
        ...
        if ($filter === 'active') {
            return $this->project->apply(['status' => 'Active'])->whereNull('archived_at');
        }
        if ($filter === 'on_hold') {
            return $this->project->apply(['status' => 'On Hold'])->whereNull('archived_at');
        }
        if ($filter === 'done') {
            return $this->project->apply(['status' => 'Done'])->whereNull('archived_at');
        }
        if ($filter === 'archived') {
            return $this->project->apply(['archived' => 1]);
        }
        return $this->project->query()->whereNull('archived_at');
    }
