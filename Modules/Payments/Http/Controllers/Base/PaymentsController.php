...
abstract class PaymentsController extends Controller
{
    ...
    protected function applyFilter()
    {
        ...
        if ($this->request->filter === 'yesterday') {
            return $this->payment->apply(['date_range' => [Carbon::parse('yesterday')->startOfDay(), Carbon::parse('yesterday')->endOfDay(), 'payment_date']])->whereNull('archived_at');
        }
        if ($this->request->filter === 'week') {
            return $this->payment->apply(['date_range' => [Carbon::parse('last monday')->startOfDay(), Carbon::parse('next friday')->endOfDay(), 'payment_date']])->whereNull('archived_at');
        }
