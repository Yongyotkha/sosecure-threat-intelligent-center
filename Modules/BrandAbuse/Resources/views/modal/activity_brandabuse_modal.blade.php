...
<div class="modal-dialog modal-dialog-aside">
    ...
    {!! Form::open(['route' => ['brandabuse.activity_save'], 'class' => 'ajaxifyForm_custom_onlythis', 'files' => false]) !!}
        <div class="modal-body">
            <input type="hidden" name="id_BrandAbuseSocialRefs" class="form-control" value="{{$BrandAbuseSocialRefs->id}}">
            @if ($site)
...
<script>
    ...
    function edit_activity(code_activity=0){    
        $.ajax({
            ...
            url: '{{ route('brandabuse.activity_get_edit_data') }}',
            type: "GET",
            data:{
    ...
    function reload_activity_history(code){
        $.ajax({
            ...
            url: '{{ route('brandabuse.activity_history_reload') }}',
            type: "GET",
            data:{
                code:code,
            },
    ...
    function delete_activity(id_activity,code_activity){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type:"DELETE",
                    url:"{{ route('brandabuse.activity_delete') }}",
                    data:{
