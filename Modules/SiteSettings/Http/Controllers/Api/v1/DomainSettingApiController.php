...
class DomainSettingApiController extends Controller
{
    /**
     * Client model
     *
     * @var \Modules\Clients\Entities\Domain
     */
    protected $Domain;
    /**
     * Request instance
    ...
    {
        $Domain = new Domain(
            $this->Domain->with(['name,descript,logo,address,remark,active'])
                ->orderByDesc('id')
                ->paginate(40)
        );
        return response($Domain, Response::HTTP_OK);
    }
    ...
    public function show($id = null)
    {
        ...
        return response(new Domain($Domain), Response::HTTP_OK);
    }

    public function save(DomainRequest $request)
    {
        // dd($request);
        ...
        $Domain->descript = $request->descript;
        $Domain->address = $request->address;
        $Domain->remark = $request->remark;
        $Domain->created_by = @Auth::user()->id;
        $Domain->active = $request->active ? 1 : 0;
        $Domain->save();
        ...
        //         ]
        //     );
        //     // $Domain->update(['primary_contact' => $user->id]);
        // }
        // if ($request->hasFile('logo')) {
        //     $this->uploadLogo($request, $Domain);
        // }
    ...
    public function change_status(DomainRequest $request)
    {
        // dd($request);
        // exit();
        $data['site_id'] = $this->request->site_id;
        $Domain = $this->Domain->findOrFail($data['site_id']);
        // $Domain->update($request->all());
        // $Domain->name = $request->name;
        $Domain->active = $Domain->active == 1 ? 0 : 1;
        $Domain->save();
    ...
    private function uploadLogo($request, $Domain)
    {
        ...
        // $currentLogo = $Domain->getOriginal('logo');
        $currentLogo = $Domain->logo;
        ...
        // dd(1234);
        // exit();
        $Domain->update(['logo' => 'logo_'.time().'.'.$img_type]);//$request->logo->hashName()
    }
    /**
